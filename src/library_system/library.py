...
class Library:
    def __init__(self):
        ...
        # Add some test books
        for idx in range(3):
            self.books.append(Book(id=idx, author=f"Author{idx}", name=f"Book{idx}"))
        ...
        if not active:
            ...
            for loan in active:
                status = (
                    "⚠️ OVERDUE"
                    if loan.is_overdue()
                    else f"Due in {loan.days_until_due()} days"
                )
...
def main():
    ...
    library.checkout_book(library.members[1], custom_book)
